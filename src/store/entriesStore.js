...
function loadInitialData() {
	try {
		...
		try {
			...
			const list = Array.isArray(seed.journalEntries) ? seed.journalEntries : [];
			save(list);
			return list;
		} catch (e) {
			return [];
...
export function getAll() { return entries.slice().sort((a, b) => new Date(a.date) - new Date(b.date)); }
...
export function update(id, patch) {
	save(entries.map((e) => (e.id === id ? normalizeEntry({ ...e, ...patch }) : e)));
}
